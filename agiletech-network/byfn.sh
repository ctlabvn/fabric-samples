...
function printHelp () {
  ...
  echo "    -c <channel name> - channel name to use (defaults to \"agiletechchannel\")"
  ...
  echo
  echo "	byfn.sh -m generate -c agiletechchannel"
  echo "	byfn.sh -m up -f docker-compose-e2e.yaml -l golang -c agiletechchannel -s couchdb"
  echo "	byfn.sh -m up -l node"
  echo "	byfn.sh -m down -c agiletechchannel"
  echo
...
function logContainerCli() {
  CLI=`docker ps -a | grep cli`
  if [ ! -z "$CLI" ]; then
    docker logs -f cli
  fi
}
...
function replacePrivateKey () {
  ...
  CURRENT_DIR=$PWD
  cd crypto-config/peerOrganizations/org1.agiletech.vn/ca/
  PRIV_KEY=$(ls *_sk)
  cd "$CURRENT_DIR"
  sed $OPTS "s/CA1_PRIVATE_KEY/${PRIV_KEY}/g" docker-compose-e2e.yaml
  cd crypto-config/peerOrganizations/org2.agiletech.vn/ca/
  PRIV_KEY=$(ls *_sk)
  cd "$CURRENT_DIR"
...
if [ "$MODE" == "up" ]; then
  ...
  # allow pause and start again instead of removing and starting over
else  
  printHelp
  exit 1
fi
