...
var instantiateChaincode = function(
    channelName,
    chaincodeName,
    chaincodeVersion,
    functionName,
    ...
    logger.debug(
        "\n============ Instantiate chaincode on organization " +
            org +
            " ============\n"
    );
    ...
    return helper
        ...
        .then(
            ...
            err => {
                logger.error(
                    "Failed to enroll user '" + username + "'. " + err
                );
                throw new Error(
                    "Failed to enroll user '" + username + "'. " + err
                );
        ...
        .then(
            results => {
                ...
                if (all_good) {
                    ...
                    helper.setEventHub(org, config.peer, eh);
                    eh.connect();
                    ...
                    return Promise.all([sendPromise].concat([txPromise]))
                        ...
                        .catch(err => {
                            logger.error(
                                util.format(
                                    "Failed to send instantiate transaction and get notifications within the timeout period. %s",
                                    err
                                )
                            );
            ...
            err => {
                logger.error(
                    "Failed to send instantiate proposal due to error: " +
                    err.stack
                        ? err.stack
                        : err
                );
        ...
        .then(
            ...
            err => {
                logger.error(
                    "Failed to send instantiate due to error: " + err.stack
                        ? err.stack
                        : err
                );
