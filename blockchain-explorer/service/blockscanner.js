...
function syncBlock() {
    ...
    let maxBlockNum;
    let curBlockNum;
    Promise.all([getMaxBlockNum(channelName), getCurBlockNum(channelName)])
        .then(datas => {
            maxBlockNum = parseInt(datas[0]);
            curBlockNum = parseInt(datas[1]) + 1;
            co(saveBlockRange, channelName, curBlockNum, maxBlockNum)
                .then(() => {
...
function* saveBlockRange(channelName, start, end) {
    while (start < end) {
        let block = yield query.getBlockByNumber(
            peer,
            channelName,
            start,
            username,
        ...
        for (let i = 0; i < txLen; i++) {
            ...
            try {
                chaincode =
                    tx.payload.data.actions[0].payload.action
                        .proposal_response_payload.extension.results.ns_rwset[1]
                        .namespace;
            } catch (err) {
                chaincode = "";
            }
            ...
            yield sql.updateBySql(
                `update chaincodes set txcount =txcount+1 where name = '${chaincode}' and channelname='${channelName}' `
            );
        }
    }
    stomp.send(
        ...
        {},
        JSON.stringify({ timestamp: new Date().getTime() / 1000, value: 0 })
    );
}

function getMaxBlockNum(channelName) {
    return query
        .getChannelHeight(peer, channelName, username, org)
        .then(data => {
            return data;
        })
        .catch(err => {
...
function* saveChaincodes(channelName) {
    let chaincodes = yield query.getInstalledChaincodes(
        peer,
        channelName,
        "installed",
        username,
    ...
    for (let i = 0; i < len; i++) {
        ...
        let c = yield sql.getRowByPkOne(
            `select count(1) as c from chaincodes where name='${chaincode.name}' and version='${chaincode.version}' and path='${chaincode.path}' and channelname='${channelName}' `
        );
        if (c.c == 0) {
...
function* savePeerlist(channelName) {
    ...
    array.forEach(function(element) {
        ...
        if (peerlists != undefined) peerlists = peerlists.concat(peerlist);
        else peerlists = peerlist;
    });
    ...
    for (let i = 0; i < peerlen; i++) {
        ...
        peers.server_hostname =
            peerlist._options["grpc.default_authority"] || "localhost";

        let c = yield sql.getRowByPkOne(
            `select count(1) as c from peer where name='${peers.name}' and requests='${peers.requests}' and server_hostname='${peers.server_hostname}' `
        );
