    ...
    if (txid != "0") {
        query
            ...
            .then(response_payloads => {
                var header =
                    response_payloads["transactionEnvelope"]["payload"][
                        "header"
                    ];
                var data =
                    response_payloads["transactionEnvelope"]["payload"]["data"];
                var signature = response_payloads["transactionEnvelope"][
                    "signature"
                ].toString("hex");
    ...
    if (txid != "0") {
        query
            ...
            .then(response_payloads => {
                ...
                var blockjsonstr = JSON.stringify(
                    response_payloads["transactionEnvelope"]
                );
...
app.post("/api/block/get", function(req, res) {
    ...
    sql
        .getRowByPkOne(
            `select blocknum ,txcount from blocks where channelname='${ledgerMgr.getCurrChannel()}' and blocknum='${number}'`
        )
        .then(row => {
            if (row) {
                res.send({
...
var server = http.listen(port, function() {
    console.log(
        `Please open Internet explorer to access ：http://${host}:${port}/`
    );
});
