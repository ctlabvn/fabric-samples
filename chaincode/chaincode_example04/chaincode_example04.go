...
// this chaincode will invoke other chaincode - invokes chaincode 02
// need correct permission, such as has the same policy
...
func (t *SimpleChaincode) invoke(stub shim.ChaincodeStubInterface, args []string) pb.Response {
  ...
  invokeArgs := su.StringArgsToBytesArgs([]string{"invoke", "a", "b", "10"})
...
func (t *SimpleChaincode) query(stub shim.ChaincodeStubInterface, args []string) pb.Response {
  ...
  if len(args) > 3 {
    ...
    queryKey := args[2]
    channelID := args[3]
    invokeArgs := su.StringArgsToBytesArgs([]string{"query", queryKey})
    response := stub.InvokeChaincode(chainCodeToCall, invokeArgs, channelID)
    if response.Status != shim.OK {
